...
3. 解析 FC 端点；首次创建的业务函数保持禁止按需调度、无预置实例。创建或更新 `<FC_PREFIX>-db-migrate`，复用 game-service ZIP、VPC、交换机与安全组。仅注入数据库连接及运行时/区域目录必需配置。
...
生产 seed 写入提示词、提示词包、运行时模板目录、LLM 步骤目录、默认套餐及免费额度，以及当前物理区域的云账号/区域/AI 端点目录。初始迁移也包含过去仅由启动补丁维护的 8 个 LLM 元数据字段。只补缺失项，保留后台修改的内容和套餐价格/启停状态。不创建演示用户、作品、大模型凭据或支付凭据。套餐被管理员删除后，只要初始化标记存在，就不重新补建。

兼容现有业务，AI 路由逻辑键仍为 `cn_shanghai`，物理部署区域取 FC_REGION，香港后台显示香港目录。目录端点由 FC API 解析，不使用 ACR；首次种子将其标为可选路由，但函数实际健康仍由后续发布检查把关，只有 AI 健康检查通过才启动 API。
