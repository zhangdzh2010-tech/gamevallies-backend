...
| 对外域名与内部 upstream | `PUBLIC_API_BASE_URL`、`USER_SERVICE_URL`、`GAME_SERVICE_URL`、`FEED_SERVICE_URL`、`AI_ENGINE_URL_CN_SHANGHAI`、`AI_ENGINE_URL_AP_SOUTHEAST_JOHOR`、`AI_ENGINE_URL`、`GAME_SERVICE_UPSTREAM_URL`、`FEED_SERVICE_UPSTREAM_URL` | 当前生产 `.env.deploy` + APIG 控制台 |
...
| AI 引擎模型 | `LLM_MODE`、`LLM_API_KEY`、`LLM_BASE_URL`、`LLM_MODEL`、`LLM_FAST_MODEL`、`PIPELINE_TIMEOUT_S` | 当前 LLM 供应商控制台 + 生产 `.env.deploy` |
