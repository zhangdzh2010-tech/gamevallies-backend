...
本节用于记录“代码已实现”和“线上已部署验证”两类状态，避免把本地改动与线上现网状态混淆。
...
- 代码已落仓并通过本地校验。
- 但对应的生产服务或生产入口配置还没有按本方案完成发布。
- 详细字段拆分见 `11.8 清晰状态总表`。

| 编号 | 当前判定 | 说明 |
|---|---|---|
| 1 | 已实现未上线 | `docker-compose.yml` 中 MySQL `connection_limit` 已从 `2` 提升到 `20`，但当前生产主链不是这份 compose。 |
| 2 | 已实现未上线 | `nginx/conf.d/default.conf` 中 AI 限流已从 `2r/s` 提升到 `30r/s`，但未确认已落到当前生产入口链路。 |
| 3 | 已实现未上线 | `RUNTIME_QA_REQUIRED` 已在代码中放宽，但 `ai-engine` 尚未按该方案重发生产。 |
| 4 | 已实现未上线 | 首轮对话快速通道已完成代码实现，但 `ai-engine` 尚未按该方案重发生产。 |
| 5 | 已实现未上线 | Prompt 合并去重已完成代码实现，但 `ai-engine` 尚未按该方案重发生产。 |
...
| 编号 | 当前判定 | 说明 |
|---|---|---|
| 14 | 部分上线待排障 | 后端 SSE 能力已完成并已部署，前端尚未正式接入；生产环境单用户 SSE 观测仍出现 `read timed out`，需继续排查 APIG / 长连接层。 |

### 11.3 未实现

| 编号 | 当前判定 | 说明 |
|---|---|---|
| 7 | 未实现 | token budget 进一步调整尚未实施。 |
| 8 | 未实现 | 更高的实体/系统上限尚未放开。 |
...
- 复核中发现 `CreationSessionRealtimeService` 在“没有任何订阅者”时也会创建并保留 `Subject`，会让未接 SSE 的会话在内存中留下无用流对象。
- 该问题已修正：现在 `publishSnapshot / publishError` 仅对已有订阅流推送，不再为无订阅会话创建流对象。
- 已恢复公开迭代入口 `POST /api/v1/games/:id/iterate`，并完成线上部署修复；生产直连验证已通过，迭代任务 `b1a22b11-52c9-4077-96c2-f5d364a4b995` 最终 `succeeded`。
- 已恢复公开分叉入口 `POST /api/v1/games/:id/fork`，并补齐事务内复制最新 bundle 的创建逻辑；该修复已完成线上部署并通过全流程 E2E 验证。
...
#### A. 全流程 creation-session E2E

- 执行脚本：`python scripts/run_live_creation_session_full_flow_e2e.py --env-file .env.deploy --timeout-s 1500 --wait-s 1800`
- 结果文件：`tmp_creation_session_e2e_batch_20260403_180558.json`
- 结果：`okCount = 0 / 5`

主要失败现象：

- 4 个 case 在迭代阶段直接命中 `POST /api/v1/games/:id/iterate -> 404`
- 1 个 case 在创建阶段失败于 `contract_qa`
- 失败任务：`f990e604-558c-4ea1-aaee-bfb402d60e56`
- 失败游戏：`80f7b8f7-bff6-43bb-a8cb-5ebde72760c5`
- 失败信息：`Generated code failed contract QA: Runtime contract requires portrait-first short-edge UI scaling; Runtime contract requires a restart entry point`

结论：

- 当前生产环境暴露的接口集与该全流程脚本预期不一致，无法把这次结果当作“新链路已完整上线”的通过依据。

#### B. 创建会话 + 生成主链单用户验证（轮询）

- 执行脚本：`python scripts/run_live_generation_concurrency.py --env-file .env.deploy --users 1 --flow session_generate --session-observer poll --diagnostics failures`
- 结果文件：`tmp_generation_concurrency_20260403_182152.json`
- 结果：`1 / 1 succeeded`
- `sessionPostLatencyMs = 144.5`
- `interactiveElapsedS = 4.691`
- `endToEndElapsedS = 157.158`

结论：

- 当前生产环境的“创建会话 + 生成”主链在轮询模式下可正常跑通。

#### C. 创建会话 + 生成主链单用户验证（SSE）

- 执行脚本：`python scripts/run_live_generation_concurrency.py --env-file .env.deploy --users 1 --flow session_generate --session-observer sse --diagnostics failures`
- 结果文件：`tmp_generation_concurrency_20260403_182128.json`
- 结果：`HTTP 404`

结论：

- 当前生产环境尚未部署 `GET /api/v1/games/creation-sessions/:sessionId/events` 这条 SSE 路由。
- 因此，SSE 相关改动目前只能确认“代码已实现并通过本地验证”，尚不能判定为“线上已生效”。

#### D. 公开迭代接口修复后的生产验证

- 部署版本：`gv-game-service` 发布版本 `119`
- 直接验证游戏：`fb31eb56-1c19-43d3-8023-72ac1c3ca291`
- 直接验证任务：`b1a22b11-52c9-4077-96c2-f5d364a4b995`
- 结果：`POST /api/v1/games/:id/iterate` 不再返回 `404`，任务最终状态 `succeeded`

结论：

- 生产环境公开迭代入口已恢复可用，之前的 `iterate -> 404` 故障已修复。

#### E. 公开 fork 接口修复后的全流程 creation-session E2E

- 执行脚本：`python scripts/run_live_creation_session_full_flow_e2e.py --env-file .env.deploy --timeout-s 1500 --wait-s 1800`
- 结果文件：`tmp_creation_session_e2e_batch_20260403_200005.json`
- 结果：`okCount = 5 / 5`
- 部署版本：`gv-game-service` 发布版本 `120`

结论：

- 创建、生成、发布、迭代、再次发布、fork、fork 发布、share-data、fork 关系查询整条主链路已在生产环境恢复并通过。
- 之前暴露出来的两个公开接口故障 `iterate -> 404` 与 `fork -> 404` 已全部修复。

#### F. SSE 路由部署后的生产验证

- 执行脚本：`python scripts/run_live_generation_concurrency.py --env-file .env.deploy --users 1 --flow session_generate --session-observer sse --diagnostics failures`
- 结果文件：`tmp_generation_concurrency_20260403_202507.json`
- 结果：`script_error`
- 失败信息：`The read operation timed out`

结论：

- `GET /api/v1/games/creation-sessions/:sessionId/events` 已不再表现为 `404` 未部署问题，但生产环境 SSE 长连接仍未稳定可用。
- 当前更像是 APIG / 代理 / 长连接层行为问题，而不是 controller 路由缺失问题。

### 11.7 当前状态结论

- 这轮诊断优化已经完成了首批和第二批中的核心高收益项，但还没有“全部实现”。
- 当前可以确认：
- 本地代码已实现并通过编译/语法验证
- 生产环境轮询主链可用
- 生产环境 create / iterate / fork 全流程主链已恢复并通过 `5 / 5` production E2E
- 生产环境 `iterate -> 404` 与 `fork -> 404` 已修复
- 生产环境 SSE 路由已部署，但单用户 SSE 观测仍存在超时，需继续排查网关长连接链路

### 11.8 清晰状态总表（以当前生产为准）

说明：

- `代码实现`：仓库代码已修改，并已完成本地编译或脚本校验。
- `生产部署`：对应服务或入口配置已经真正发布到当前生产链路。
- `生产验证`：已经有现网脚本、接口探测或 E2E 结果支撑。
- 当前中国区生产主链是 `APIG + VeFaaS`，因此 repo 中的 `docker-compose.yml` / `nginx/conf.d/default.conf` 改动不会自动等于“已上线”。

| 编号 | 方案项 | 代码实现 | 生产部署 | 生产验证 | 当前判定 |
|---|---|---|---|---|---|
| P0-1 | `connection_limit=2 -> 20` | 已完成 | 未部署到当前生产链路 | 无 | 已实现未上线 |
| P0-2 | `ai_limit rate=2r/s -> 30r/s` | 已完成 | 未部署到当前生产入口链路 | 无 | 已实现未上线 |
| P0-3 | `RUNTIME_QA_REQUIRED=false` | 已完成 | `ai-engine` 尚未按本方案重发生产 | 无 | 已实现未上线 |
...
| 项目 | 代码实现 | 生产部署 | 生产验证 | 当前判定 |
|---|---|---|---|---|
| 公开迭代入口 `POST /api/v1/games/:id/iterate` 恢复 | 已完成 | 已完成 | 已通过生产直连验证，任务 `b1a22b11-52c9-4077-96c2-f5d364a4b995` 成功 | 已上线已验证 |
| 公开分叉入口 `POST /api/v1/games/:id/fork` 恢复 | 已完成 | 已完成 | 已通过生产全流程 E2E | 已上线已验证 |
| create / generate / iterate / fork 全链路回归 | 不适用 | 不适用 | `tmp_creation_session_e2e_batch_20260403_200005.json`，`okCount = 5 / 5` | 已验证通过 |

### 11.10 新增产品决策落地与复核结果（2026-04-03 晚）

本节用于记录本轮按最新产品决策追加落地的两项修复：

- `POST /api/v1/games/generate` 明确保留对外，因此已恢复公开路由并重新部署生产。
- creation-session 明确不允许在 `initializing` 阶段直接生成，因此已下沉为后端硬约束，不再只依赖前端等待。

#### A. legacy 公开生成入口恢复

- 决策：`POST /api/v1/games/generate` 继续保留对外。
- 代码变更：`game.controller.ts` 已重新暴露公开生成入口，调用现有 `gameService.create(...)` 主流程。
- 生产部署：已随 `gv-game-service` 发布版本 `121` 上线。
- 生产验证：
- 已对 `POST /api/v1/games/generate` 做现网直连探测，返回 `HTTP 201`，接口恢复可用。
- 已重新执行 `python scripts/run_live_generation_e2e.py --env-file .env.deploy`。
- 结果文件：`tmp_e2e_live_20260403_212322.json`
- 结果：`full_flow_success = 5 / 5`

结论：

- legacy 公开生成入口已经恢复，不再是 `404`。
- 文档、测试脚本和线上实际暴露接口重新对齐。

#### B. creation-session 禁止初始化未完成直接生成

- 决策：creation-session 不允许在 `initializing` 阶段直接生成。
- 代码变更：`generateFromSession(...)` 已改为复用 `assertSessionMutable(...)`，后端统一拒绝 `initializing / generating / abandoned / completed` 等非法状态。
- 生产部署：已随 `gv-game-service` 发布版本 `121` 上线。
- 生产验证：
- 已对新创建会话做“初始化完成前立即 generate”直连探测。
- 结果：`POST /api/v1/games/creation-sessions/:sessionId/generate` 返回 `HTTP 409`
- 失败信息：`Creation session is still initializing`

结论：

- “初始化未完成也能直接生成”的后端漏洞已关闭。
- 该约束现在由后端硬执行，不再依赖前端是否等待。

#### C. 按真实对话旅程复跑 creation-session E2E

- 脚本修正：`scripts/run_live_creation_session_full_flow_e2e.py` 已补充等待 session 进入 `readyToGenerate` 或拿到 `currentQuestion`，不再在初始化阶段直接 generate。
- 执行脚本：`python scripts/run_live_creation_session_full_flow_e2e.py --env-file .env.deploy --timeout-s 1500 --wait-s 1800`
- 结果文件：`tmp_creation_session_e2e_batch_20260403_212322.json`
- 结果：`okCount = 4 / 5`

分项结果：

- `office_slacker_cn`：通过，session 在约 `4.707s` 进入可交互状态。
- `circuit_classroom_cn`：通过，session 在约 `4.728s` 进入可交互状态。
- `parkour_delivery_en`：通过，session 在约 `4.717s` 进入可交互状态。
- `history_quiz_show_cn`：通过，session 在约 `4.720s` 进入可交互状态。
- `fruit_merge_relax_en`：失败，创建任务 `4f87b0c6-e6bb-4849-9fbf-d7c7349309de` 在 `runtime_simulation_qa` 阶段失败，失败家族为 `runtime_qa`，错误信息为 `Generated code failed runtime QA: Runtime JS error: Cannot read properties of undefined (reading 'undefined')`

结论：

- 这轮复跑已经真实覆盖“创建会话 -> 等待互动 -> 补充信息 -> 生成”的用户旅程，不能再把它当成旧版脚本那种“跳过提问也算通过”的结果。
- 当前该旅程的系统性接口问题已经修复，但仍存在单个生成任务在 runtime QA 阶段失败的内容级个案，需单独跟进。
