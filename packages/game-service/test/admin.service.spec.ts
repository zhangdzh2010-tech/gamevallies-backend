...
describe("AdminService", () => {
  ...
  beforeEach(() => {
    ...
    prisma.$transaction.mockImplementation(async (callback: (tx: any) => any) =>
      callback(prisma),
    );
    configService = {
  ...
  it("synchronizes prompt configs from the checked-in catalog and refreshes prompt caches", async () => {
    ...
    prisma.systemConfig.findUnique.mockImplementation(
      async ({ where }: { where: { configKey: string } }) => {
        const entry = defaults.find((item) => item.key === where.configKey);
        if (!entry) {
          return null;
        }
        if (where.configKey === targetPrompt!.key) {
          return {
            id: "cfg-prompt-stale",
        ...
        return {
          id: `cfg-${where.configKey}`,
          configKey: where.configKey,
          configValue: typeof entry.value === "string" ? entry.value : "",
    ...
    prisma.systemConfig.update.mockResolvedValue({
      id: "cfg-prompt-stale",
      configKey: targetPrompt!.key,
      configValue: targetPrompt!.value,
    ...
    const result = await service.initDefaultPrompts();

    expect(prisma.systemConfig.create).not.toHaveBeenCalled();
    expect(prisma.systemConfig.update).toHaveBeenCalledTimes(1);
    expect(prisma.systemConfig.update).toHaveBeenCalledWith({
      where: { configKey: targetPrompt!.key },
      data: {
        configValue: targetPrompt!.value,
        description: targetPrompt!.description ?? null,
        category: "prompt",
      },
    });
    expect(result).toEqual(
  ...
  it("upserts business timeout configs by expanding to backing timeout keys", async () => {
    ...
    prisma.systemConfig.findMany.mockResolvedValue([
      ...
      {
        id: "cfg-timeout-2",
        configKey: "timeout.ai_engine.iterate.param_adjust_overall_s",
        configValue: "180",
        category: "timeout",
        createdAt: new Date("2026-04-15T00:00:00.000Z"),
        updatedAt: new Date("2026-04-15T00:05:00.000Z"),
      ...
      {
        id: "cfg-timeout-5",
        configKey: "timeout.ai_engine.iterate.mechanic_change_request_s",
        configValue: "135",
        category: "timeout",
        createdAt: new Date("2026-04-15T00:00:00.000Z"),
        updatedAt: new Date("2026-04-15T00:05:00.000Z"),
    ...
    const result = await service.upsertConfig(
      "timeout.business.iterate_generation_s",
      {
        value: "240",
        category: "timeout",
      },
    ...
    expect(prisma.systemConfig.upsert).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({
        where: {
          configKey: "timeout.ai_engine.iterate.param_adjust_request_s",
  ...
  it("builds prompt pipeline payload with five core steps and live bundle/runtime sections", async () => {
    ...
    prisma.promptBundle.findMany.mockResolvedValue([
      {
        ...
        profileOverrides: {
          logic_generate: {
            state_flow: "boot -> ready -> playing -> game_over",
          },
        },
    ...
    expect(result.steps[0].prompts).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          configKey: "prompt.slot_json_repair_user_template",
          variables: ["{source_text}", "{raw_parser_output}"],
        }),
      ]),
    );
    expect(result.steps[0].prompts).not.toEqual(
      expect.arrayContaining([
        expect.objectContaining({
    ...
    expect(result.steps[1].support[0]).toEqual(
      expect.objectContaining({
    ...
    expect(result.steps[2].support[0]).toEqual(
      expect.objectContaining({
    ...
    expect(result.steps[2].prompts).not.toEqual(
      expect.arrayContaining([
        expect.objectContaining({
    ...
    expect(result.extras).not.toEqual(
      expect.arrayContaining([expect.objectContaining({ id: "extra_tools" })]),
    );
  });
  ...
  it("returns dashboard subscription overview with a date range filter", async () => {
    ...
    prisma.game.aggregate
      .mockResolvedValueOnce({
        _sum: { playCount: 1234, likeCount: 88, forkCount: 12 },
      })
        ...
        generationTasks: [
          {
            ...
            resultSummary: {
              ...
              iterationRetries: 1,
              qaWarnings: [
                { type: "runtime_qa_unavailable", severity: "warning" },
              ],
  ...
  it("hashes admin-created user passwords with bcrypt", async () => {
    ...
    await expect(bcrypt.compare("SmokePay123456", passwordHash)).resolves.toBe(
      true,
    );
  });
  ...
  it("hashes reset passwords with bcrypt", async () => {
    ...
    await expect(bcrypt.compare("ResetPass123", passwordHash)).resolves.toBe(
      true,
    );
  });
  ...
  it("derives provider region fields from the selected region target", async () => {
    ...
    const refreshSpy = jest
      .spyOn(service, "refreshLlmGateway")
  ...
  it("returns the raw provider secrets only through the explicit admin secret endpoint", async () => {
    prisma.llmGatewayProvider.findUnique.mockResolvedValue({
      id: "provider-1",
      apiKey: "sk-test-1234",
      extraConfig: {
    ...
    await expect(
      service.getLlmProviderSecret("provider-1", "api-key"),
    ).resolves.toEqual({
      providerId: "provider-1",
      kind: "api-key",
      value: "sk-test-1234",
    ...
    await expect(
      service.getLlmProviderSecret("provider-1", "catalog-api-key"),
    ).resolves.toEqual({
      providerId: "provider-1",
      kind: "catalog-api-key",
      value: "catalog-secret-1234",
    });
  });

  it("rejects unsupported provider secret kinds", async () => {
    await expect(
      service.getLlmProviderSecret("provider-1", "raw"),
  ...
  it("proxies provider catalog preview to the selected ai-engine region", async () => {
    ...
    try {
      ...
      expect(result.resolvedCatalogApiUrl).toBe(
  ...
  it("batch updates game statuses and reports missing ids", async () => {
    ...
    const result = await service.batchUpdateGameStatus(
      ["game-1", "game-2", "game-missing"],
      "published",
    );
  ...
  it("batch deletes games after terminating active tasks", async () => {
    ...
    const result = await service.batchDeleteGames([
      "game-1",
      "game-2",
      "game-3",
    ]);
  ...
  it("keeps deployed targets selectable for providers when a resolved env endpoint exists", async () => {
    ...
    const result = await service.listAiEngineRegionTargets({
      providerSelectableOnly: true,
    });
  ...
  it("surfaces a clear refresh error when every llm gateway admin endpoint fails", async () => {
    ...
    const postSpy = jest
      .spyOn(axios, "post")
      .mockRejectedValueOnce(
        Object.assign(new Error("configured failure"), {
          isAxiosError: true,
          response: {
            data: { detail: "configured gateway unavailable" },
          },
      ...
      .mockRejectedValueOnce(
        Object.assign(new Error("stale failure"), {
          isAxiosError: true,
          response: {
            data: { message: "region runtime unreachable" },
          },
