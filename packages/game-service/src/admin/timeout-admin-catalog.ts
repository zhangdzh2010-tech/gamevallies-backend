...
const SECTION_GLOBAL: VisibleTimeoutSectionMeta = {
  ...
  title: "全局守卫",
  description:
    "全流程总超时作为唯一的全局总闸门，其他 transport / relay / cache 细项保持默认。",
};
...
const BUSINESS_TIMEOUT_CATALOG: VisibleTimeoutCatalogEntry[] = [
  {
    ...
    readCurrentValue: (reader) =>
      reader.current("timeout.ai_engine.intent_parse.overall_s"),
  ...
  },
  {
    ...
    displayName: "创建主生成超时（code_generate.full）",
    description:
      "控制首次创建主生成步骤 code_generate.full 的长请求预算，直接影响 create 主链路的成功率、等待时长和成本。",
    unit: "s",
    valueType: "int",
    minValue: 60,
    maxValue: 600,
    ...
    readCurrentValue: (reader) =>
      reader.current("timeout.ai_engine.llm_long_generation_s"),
    readDefaultValue: (reader) =>
      reader.default("timeout.ai_engine.llm_long_generation_s"),
    buildUpdates: (value) => [
      {
  ...
  },
  {
    ...
    readCurrentValue: (reader) =>
      reader.current("timeout.ai_engine.qa_repair_s"),
    readDefaultValue: (reader) =>
  ...
  },
  {
    ...
    displayName: "运行时验收超时",
    description:
      "控制运行时验收的总上限；phase、bonus、wait ratio 等内部细项维持默认策略。",
    unit: "s",
    valueType: "int",
    minValue: 10,
    maxValue: 120,
    ...
    readCurrentValue: (reader) =>
      reader.current("timeout.ai_engine.runtime_qa.max_s"),
    readDefaultValue: (reader) =>
  ...
  },
  {
    ...
    displayName: "迭代生成超时",
    description:
      "控制 iterate 的改写阶段总预算，内部自动映射到参数调整、元素修改和机制改写。",
    unit: "s",
    valueType: "int",
    minValue: 90,
    maxValue: 480,
    ...
    readCurrentValue: (reader) =>
      reader.current("timeout.ai_engine.iterate.element_change_overall_s"),
    readDefaultValue: (reader) =>
  ...
  },
  {
    ...
    displayName: "全流程总超时",
    description:
      "控制 create / iterate 的全流程总预算，并同步作为 V2 主链路的最小超时下限。",
    unit: "s",
    valueType: "int",
    minValue: 60,
    maxValue: 3600,
...
export function getBusinessTimeoutConfig(
  key: string,
  configs: TimeoutRecordLike[],
) {
  return (
    listBusinessTimeoutConfigs(configs).find(
      (entry) => entry.configKey === key,
    ) || null
  );
}

export function expandBusinessTimeoutConfigUpdates(
  key: string,
  rawValue: string,
): VisibleTimeoutUpdate[] {
  const entry = BUSINESS_TIMEOUT_CATALOG_BY_KEY.get(key);
  if (!entry) {
    throw new Error(`Unknown business timeout key: ${key}`);
  }
  const numericValue = parseBusinessTimeoutValue(entry, rawValue);
  return entry.buildUpdates(numericValue);
}

function parseBusinessTimeoutValue(
  entry: VisibleTimeoutCatalogEntry,
  rawValue: string,
): number {
  const normalized = String(rawValue ?? "").trim();
  if (!/^-?\d+$/.test(normalized)) {
    throw new Error(
      `${entry.displayName} must be an integer number of seconds.`,
    );
  }
  const numericValue = Number.parseInt(normalized, 10);
  if (!Number.isFinite(numericValue)) {
    throw new Error(`Invalid timeout value for ${entry.key}: ${rawValue}`);
  }
  if (numericValue < entry.minValue || numericValue > entry.maxValue) {
    throw new Error(
      `${entry.displayName} must be between ${entry.minValue} and ${entry.maxValue} ${entry.unit}.`,
    );
  }
  return numericValue;
}
...
function pickDate(
  ...
  const timestamp =
    mode === "min" ? Math.min(...timestamps) : Math.max(...timestamps);
