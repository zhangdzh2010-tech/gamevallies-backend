...
class CodeGenerationPromptsMixin:
    ...
    def _build_preflight_safety_block(
        ...
        lines = [
            ...
            "- Declare `update()`, `render()`, `resetGame()`/`restartGame()`, and any helper used by `init()`, listeners, or rAF (`resize`, `loop`, `initGrid`, `getEventPos`, `getCellAt`, `isAdjacent`, …) as function declarations (`function resetGame() {}`, `function initGrid() {}`, `function update() {}`). Do not use `const name = () =>`, `let name = function`, object-method shorthand (`initGrid() {` inside `{ ... }`), or `name: () =>` / `name: function` properties unless a same-scope `function name()` also exists — methods, object properties, and `const`/`let` arrows throw TDZ or leave free `initGrid()`/`getCellAt()`/`resetGame()`/`update()` calls undefined. If a frame callback reads `t`, write `function loop(t)` / `function update(t)` or `let t = 0` before `update(t)` / `if (t > last)`. Declare short neighbor aliases (`let nr, nc;` or `const [nr, nc] = …`) and live counters (`let combo = 0;`) before reading or incrementing them.",
            ...
            "- Keep host object calls dotted: write `ctx.beginPath()`, `ctx.fill()`, `canvas.getContext('2d')`, never a concatenated identifier such as `ctxbeginPath()`.",
            "- Declare live state flags and multipliers before comparing or passing them (`let phase = 'ready'`, `let multiplier = 1`). Do not read undeclared names in `=== 'playing'` checks or as live arguments.",
