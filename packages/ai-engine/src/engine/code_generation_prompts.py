...
class CodeGenerationPromptsMixin:
    ...
    def _build_preflight_safety_block(
        ...
        lines = [
            ...
            "- Declare `update()`, `render()`, `resetGame()`/`restartGame()`, and any helper used by `init()`, listeners, or rAF (`resize`, `loop`, …) as function declarations (`function resetGame() {}`, `function update() {}`). Do not use `const name = () =>`, `let name = function`, or object-method shorthand (`resetGame() {` inside `{ ... }`) unless a same-scope `function resetGame()` also exists — methods and `const`/`let` arrows throw TDZ or leave free `resetGame()`/`update()` calls undefined. If a frame callback reads `t`, write `function loop(t)` / `function update(t)` or `let t = 0` before `update(t)` / `if (t > last)`. Declare short neighbor aliases (`let nr, nc;` or `const [nr, nc] = …`) and live counters (`let combo = 0;`) before reading or incrementing them.",
