...
class CodeGenerationPromptsMixin:
    ...
    def _build_preflight_safety_block(
        ...
        lines = [
            ...
            "- Declare `update()`, `render()`, `resize()`, and `loop()` as function declarations before the first direct call, `addEventListener('resize', resize)`, or `requestAnimationFrame(loop)`. Do not use `const resize = () =>` or `let loop = function` — those bindings throw TDZ if init runs first.",
