...
光合示意中，光线从光源/太阳指向叶片中心，不要画成与叶片无关的交叉装饰线。「光」等单字圆标须足够大、行高居中，整字留在圆内，不要紧贴圆角卡片边缘以免裁切。
单摆摆线终点必须落在摆球圆心；用中心坐标画弧或把 CSS 摆球 translate(-50%,-50%) / 负一半半径回拉，不要把弦接到圆的左上角。
...
def science_runtime_repair_guidance(issues: list[str]) -> str:
    ...
    if re.search(r'光合|photosynthesis|产氧|光照', text, re.I):
        hints.append(
            ...
            '光线从太阳指向叶片中心；「光」圆标用居中文字与足够直径，避免裁切字形。'
            '补丁须替换完整语句；不要把 if 拼进 phase += dt 等表达式中间，避免 Unexpected token if。')
    if re.search(r'单摆|摆球|pendulum|bob', text, re.I):
        hints.append(
            '单摆：摆线终点与摆球圆心共用同一坐标。Canvas 用 arc(x,yb)；'
            'DOM/SVG 不要按摆球左上角定位，应回拉半径或 transform 居中。')
    return '\n'.join(hints)
