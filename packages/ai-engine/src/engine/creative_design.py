...
def core_playability_contract(spec: Any) -> str:
    ...
    return "\n".join([
        ...
        "Use one requestAnimationFrame clock: movement, spawning, difficulty and countdown advance by elapsed seconds, never pixels per frame or an independent setInterval timer.",
        "Clamp long frame gaps; pause freezes simulation and countdown, resume resets the previous timestamp, and restart resets clock, inputs and all entities.",
        "Draw and hit-test in the same logical coordinate system. Convert pointer coordinates exactly once; do not apply a canvas scale or entity translation twice.",
