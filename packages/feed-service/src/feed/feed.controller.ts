...
export class FeedController {
  ...
  async getTrendingFeed(@Query() pagination: FeedPaginationDto) {
    const result = await this.feedService.getTrendingFeed(pagination.page, pagination.limit);
    return ok(toPage({
  ...
  async getLatestFeed(@Query() pagination: FeedPaginationDto) {
    const result = await this.feedService.getLatestFeed(pagination.page, pagination.limit);
    return ok(toPage({
      data: result.data.map((game: any) => presentGame(game)),
      pagination: result.pagination,
    }));
  ...
  async getFeaturedFeed(@Query() pagination: FeedPaginationDto) {
    const result = await this.feedService.getFeaturedFeed(pagination.page, pagination.limit);
    return ok(toPage({
      data: result.data.map((game: any) => presentGame(game)),
      pagination: result.pagination,
    }));
  ...
  async getFeedByCategory(@Query() query: FilteredFeedDto) {
    ...
    const result = await this.feedService.getFeedByType(category || 'casual', query.page, query.limit);
    return ok(toPage({
      data: result.data.map((game: any) => presentGame(game)),
      pagination: result.pagination,
    }));
